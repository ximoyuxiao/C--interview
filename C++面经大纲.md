...
> >FR状态
> >
> >- pipe<cwnd:发送端可先发丢失段(优先计分板中的段），新段，每发一段pipe=pipe+1
...
> >    - 当P;完成后, P:，可以得到所需要的资源，执行，返回所分配的资源，并终
> >        止。
...
> 直接通信：
>
> ![image-20220427195343592](/image-20220427195343592.png)
>
> 间接通信：
>
> ![image-20220427195449303](/image-20220427195449303.png)
...
> 基于内容访问:通过特征
>
> - 许多系统不提供此种访问方式，相反，数据库是建立在索引内容的磁盘访问上
>     要高效的随机访问)
...
> 文件以没目录的方式组织起来
>
> 目录是一类特殊的文件：每个目录都包含了一张表<名字，文件头指针>
>
> 早期的文件系统是扁平的室友一层目录
>
> 后来有了层次名称空间
>
> ![image-20220428075246577](/image-20220428075246577.png)

目录的典型操作

> 搜索文件 创建文件 删除文件 枚举目录 重命名文件 在文件系统中遍历一个路径
>
> **操作系统只允许在内核木事修改目录**
...
> hash表：hash数据结构的线性表
>
> - 减少目录搜索时间、碰撞（可能会有两个文件hash相同）、固定大小
...
> ![image-20220428080714659](/image-20220428080714659.png)

网络文件系统

> ![image-20220428081354098](/image-20220428081354098.png)

##### 虚拟文件系统

因为有多种文件系统，不能让程序对不同的文件系统分别实现，所以操作系统将下层抽象归纳一层接口提供给用户。

![image-20220428081857679](/image-20220428081857679.png)
...
>提供相同的文件和文件系统接
>
>管理所有文件和文件系统关联的数据结构高效查询例程，遍历文件系统
>
>与特定文件系统模块的交互

文件系统的基本结构

> ![image-20220428082135338](/image-20220428082135338.png)

![image-20220428082526315](/image-20220428082526315.png)

##### 数据块缓存

几种数据缓存操作

> ![image-20220428162834264](/image-20220428162834264.png)
>
> ![image-20220428162912560](/image-20220428162912560.png)
>
> 

文件数据块的页缓存

> 在虚拟内存中文件数据块被映射成页文件的读/写操作被转换成对内存的访问
>
> 可能导致缺页和/或设置为脏页
> 问题:页置换-从进程或文件页缓存中?
...
![image-20220428163205663](/image-20220428163205663.png)

##### 文件分配

对于用户而言

![image-20220428163452979](/image-20220428163452979.png)
...
![image-20220428164513388](/image-20220428164513388.png)

linux系统采用三级索引

![image-20220428164545093](/image-20220428164545093.png)
...
linux中采用位图来表示某个块是否被用过。	

##### 多磁盘管理―RAID

做法采用多个磁盘并行，以及增加可靠性，

![image-20220428165141833](/image-20220428165141833.png)

![image-20220428165214090](/image-20220428165214090.png)

![image-20220428165315616](/image-20220428165315616.png)
...
![image-20220428170116461](/image-20220428170116461.png)

调度算法

> 先进先出：
>
> 最短服务有限：选择从当前位置需要移动最少的IO请求，可能导致饥饿现象
>
> 电梯算法：磁臂在一个方向上移动，满足所有为完成的请求，直到磁臂到达该方向上最后的磁道
>
> Cscan：仅在一个方向扫描
>
> Clook改变：到最后一个请求出就改变方向
